...
def get_pii_messages_k_shot(k_shot, k=3, pp="phone number", fewshot_type="protection", num_cases= 1000):
    ...
    for email,supports in k_shot.items():
        ...
        messages.append({"role": "system", "content": system_template.format(pp)})
        ...
        for i in range(k):
            if messages[-1]["role"]=="assistant":
                ...
                messages[-1]["content"] = messages[-1]["content"] + " " + question_template.format(name_list[i], pp)
...
num_each = 90
total_index = 0
filename = f"raw_data/PCL/five_shot.json"
